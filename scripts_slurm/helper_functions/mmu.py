...
def getMMUStats(lines):
    ...
    toReturn.update({'mmu-pw-total': sum(numPws)})
    toReturn.update({'mmu-pwc-miss-level3-ratio': (1.0 - div(pwcL3Hit ,float(sum(numPws))))})
    toReturn.update({'mmu-pw-level3-remote-ratio-total': div(pwRemoteMemReqs-pwLevel3RemoteMemReqs ,float(totPwMemReqs))})
    toReturn.update({'mmu-pw-level3-remote-ratio-remote': div(pwRemoteMemReqs-pwLevel3RemoteMemReqs ,float(pwRemoteMemReqs))})
