...
func (pt *PageTableImpl) NewPageTable(log2PageSize uint64) *PageTableImpl {
...
func (pt *PageTableImpl) getTable(pid ca.PID) *processHashTableImpl {
	pt.Lock()
	defer pt.Unlock()
...
func (pt *PageTableImpl) GetRoot(pid ca.PID, chiplet uint64) uint64 {
...
func newProcessHashTable(log2PageSize uint64, tableSize uint64, a MemoryAllocator) *processHashTableImpl {
	...
	t.tableSize = tableSize // sbin: # hpt entries in log2 (each 128B)
	t.hptEntrySize = uint64(4)
	t.offsetMask = ^(^uint64(0) << t.hptEntrySize)
	t.memAllocator = a
	t.entries = list.New()
	t.entriesTable = make(map[uint64]*list.Element)

	t.baseAddr = make([]uint64, 4) // sbin: per chiplet hashed page table base address
	for i := range t.baseAddr {
		pagesPerChiplet := uint64(1)<<((t.tableSize+t.hptEntrySize+3)-t.log2PageSize)
		fmt.Println("Pages per chiplet in HPT: %d", pagesPerChiplet)
		for j := 0; j < int(pagesPerChiplet); j++ {
			a.allocateHashedPageTable(0, 0, i)
		}
	}
	...
	for i := range t.root {
		...
		for j := range t.root[i] {
			e := new(hptEntry)
			e.pAddr = t.baseAddr[i] + (uint64(j))*(uint64(1)<<(t.hptEntrySize+3))
			e.vpnTag = ^uint64(0)
			e.valid = false
			e.deleted = false
			e.children = make([]*pte, uint64(1)<<t.hptEntrySize)
			t.root[i][j] = e
		}
	}
