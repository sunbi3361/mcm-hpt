...
func (b *Benchmark) initMem() {
	...
	if b.useUnifiedMemory {
		...
		b.gInput = b.driver.AllocateMemoryLASP(b.context, uint64(b.Length), "div4")
	} else {
	...
	for i, gpu := range b.gpus {
		...
		if b.useUnifiedMemory {
			...
			b.gExpandedKey[i] = b.driver.AllocateMemoryLASP(
				b.context, uint64(len(b.expandedKey)*4), "div4")
			b.gS[i] = b.driver.AllocateMemoryLASP(b.context, uint64(len(b.s)), "div4")
		} else {
	...
	fmt.Printf("Footprint: %.3f MB\n",
		float64(uint64(b.Length)+(uint64(len(b.expandedKey)*4)+uint64(len(b.s))))/1024.0/1024.0)
