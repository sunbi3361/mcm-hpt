...
func (m *GPUMatrixMultiplier) initMemory(
	...
	if m.useLASPMemoryAlloc {
		gA := m.driver.AllocateMemoryLASP(m.context, uint64(mA.Width*mA.Height*4), "div4")
		gB := m.driver.AllocateMemoryLASP(m.context, uint64(mB.Width*mB.Height*4), "div4")
		gC := m.driver.AllocateMemoryLASP(m.context, uint64(mC.Width*mC.Height*4), "div4")
		fmt.Printf("Footprint: %.3f MB\n",
			float64(uint64(mA.Width*mA.Height*4)+uint64(mB.Width*mB.Height*4)+uint64(mC.Width*mC.Height*4))/1024.0/1024.0)
